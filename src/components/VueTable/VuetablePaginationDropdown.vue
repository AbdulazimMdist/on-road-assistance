<template>
  <div :class="[$_css.wrapperClass]">
    <a @click="loadPage('prev')" :class="[$_css.linkClass, { [$_css.disabledClass]: isOnFirstPage }]">
    ...
    <select :class="['vuetable-pagination-dropdown', $_css.dropdownClass]" @change="loadPage($event.target.selectedIndex + firstPage)">
      <option v-for="(n, i) in totalPage" :key="n" :class="[$_css.pageClass]" :value="i + firstPage" :selected="isCurrentPage(i + firstPage)">{{ pageText }} {{ n }}</option>
    </select>
    <a @click="loadPage('next')" :class="[$_css.linkClass, { [$_css.disabledClass]: isOnLastPage }]">
