<template>
  <div class="relative rounded" :class="$_css.tableWrapper">
        ...
        <thead>
          <slot name="tableHeader" :fields="tableFields">
            <template v-for="(header, headerIndex) in headerRows">
              <component :is="header" :key="headerIndex" @vuetable:header-event="onHeaderEvent"></component>
            </template>
          </slot>
        </thead>
      </table>
    ...
    <div class="vuetable-body-wrapper" :class="{ 'fixed-header': isFixedHeader }" :style="{ height: tableHeight }">
      <table :class="['vuetable', isFixedHeader ? 'fixed-header' : '', $_css.tableClass, $_css.tableBodyClass]">
        <vuetable-col-group />
        <thead v-if="!isFixedHeader" class="bg-gray-400">
          <slot name="tableHeader" :fields="tableFields">
            <template v-for="(header, headerIndex) in headerRows">
              <component :is="header" :key="headerIndex" @vuetable:header-event="onHeaderEvent"></component>
            </template>
          ...
          <template v-for="(item, itemIndex) in tableData">
              ...
              <template v-for="(field, fieldIndex) in tableFields">
                <template v-if="field.visible">
                  ...
                  <template v-else-if="isFieldSlot(field.name)">
                    <td :class="bodyClass('vuetable-slot', field)" :key="fieldIndex" :style="{ width: field.width }">
                      <slot :name="field.name" :row-data="item" :row-index="itemIndex" :row-field="field"></slot>
                    </td>
                  </template>
                  <template v-else>
                    <td
            ...
            <template v-if="useDetailRow">
              <transition :name="detailRowTransition" :key="itemIndex">
                <tr v-if="isVisibleDetailRow(item[trackBy])" @click="onDetailRowClick(item, itemIndex, $event)" :class="onDetailRowClass(item, itemIndex)">
                  <td :colspan="countVisibleFields">
                    <component :is="detailRowComponent" :row-data="item" :row-index="itemIndex" :options="detailRowOptions"></component>
                  </td>
                </tr>
              </transition>
            </template>
    ...
    <div v-if="loading" class="absolute top-0 bottom-0 left-0 right-0 z-50 bg-white bg-opacity-60">
      <line-loading />
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    fetch(apiUrl, httpOptions) {
      ...
      if (this.httpMethod === "get") {
        ...
        return this.$axios.post(apiUrl, params, httpOptions);
      }
    },
    ...
    gotoNextPage() {
      if (this.currentPage + 1 < Math.ceil(this.tablePagination.total / this.perPage)) {
        this.currentPage++;
        this.loadData();
      }
    },
    ...
    gotoPage(page) {
      if (page != this.currentPage && page >= this.firstPage && page <= Math.ceil(this.tablePagination.total / this.perPage)) {
