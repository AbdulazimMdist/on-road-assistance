    ...
    <template v-for="(field, fieldIndex) in vuetable.tableFields">
      <template v-if="field.visible">
        ...
        <template v-else-if="vuetable.isFieldSlot(field.name)">
          <th :class="headerClass('vuetable-ыth-slot', field)" :key="fieldIndex" :style="{ width: field.width }" v-html="renderTitle(field)" @click="onColumnHeaderClicked(field, $event)"></th>
        </template>
        <template v-else>
          <th
            @click="onColumnHeaderClicked(field, $event)"
  ...
  methods: {
    ...
    renderTitle(field) {
      ...
      if ((title.length > 0 && this.isInCurrentSortGroup(field)) || this.hasSortableIcon(field)) {
        ...
        return '<div class="flex justify-between">' + title + " " + iconTag + "</div>";
      }
    ...
    renderIconTag(classes, options = "") {
      return typeof this.css.renderIcon === "undefined" ? `<i class="${classes.join(" ")} ml-3" ${options}></i>` : this.css.renderIcon(classes, options);
    },
